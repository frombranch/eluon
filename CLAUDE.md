...
> `core`는 v1.12.0에서 **테마 목록에서 뺐습니다.** `tokens/core.json`은 그대로 있습니다 — 모든 테마가 `extends: core`로 이 파일 위에 얹히는 파운데이션이라 지울 수 없습니다. 뺀 것은 "브랜드 없이 파운데이션만 렌더한 결과물"이고, 실제로 고를 일이 없어 혼란만 줬습니다.

> ⚠️ `ember`는 **가명이라 이름과 색이 다릅니다** — 잉걸불이라는 이름과 달리 실제 브랜드색은 딥그린 `#12503C`입니다. 브랜드색은 언제나 `--color-brand-primary`에서 읽으십시오. (v1.12.0에서는 이 테마 id가 `amber`였는데, `core.json`의 `primitive.amber`(경고색 램프)와 이름이 겹쳐 v1.12.0에서 바꿨습니다.)
