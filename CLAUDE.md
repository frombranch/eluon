...
⚠️ **외부 리소스를 못 불러오는 환경에 넘길 페이지라면 경로 C.**

### 경로 C — 저장소 클론 (시각 검수·단일 파일)
```bash
git clone --depth 1 --branch v1.12.3 https://github.com/frombranch/eluon.git
```
이미지를 실제로 봐야 할 때, 또는 외부 링크 없이 단일 HTML 파일로 넘겨야 할 때만
(토큰 CSS는 `<style>`에 통째로, 이미지는 base64 data: URI로 인라인).
