...
index/sheet-<theme>.png    ← [생성됨] 엘루온 시트
...
| `noir` | 고객사 테마(가명). 순검정 `#000000`, 라운드 전면 0 | 해당 고객사 건 |
| `linen` | 고객사 테마(가명). 따뜻한 먹색 `#1B1A18` + 딥브라운 `#3B2412`, 굵기 400 중심 | 해당 고객사 건 |
...
```html
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/frombranch/eluon@v1.12.4/docs/tokens/eluon-eluo.css">
<style>
.btn-primary-lg{
...
```bash
git clone --depth 1 --branch v1.12.4 https://github.com/frombranch/eluon.git
```
...
**사이트에서 자산 후보 찾기 — 고객사 건의 첫 관문 (건너뛰지 않는다)**

전제: **사이트는 이미 열려 있고 우리는 그 운영을 맡는다.** 새로 짓는 게 아니다.
그래서 시작 입력은 브리프가 아니라 **URL**이다. 없으면 물어본다.

1. 주소를 열어 **두 가지를 동시에** 뜬다
   - **값** — computed style 로 색·활자·치수·라운드·컨테이너 (→ 테마)
   - **구조** — 반복되는 UI 조각과 **반복 횟수** (→ 자산)
2. **이 사이트가 무엇으로 먹고사는지 먼저 지목한다.**
   호텔=예약 · 커머스=구매 · 미디어=열람 · 채용=지원 · 금융=조회.
   **그 흐름의 부품은 다른 무엇보다 먼저 센다. 빠지면 안 되는 것들이다.**
3. `manifest.json`과 대조해 표 두 개로 — 이미 있는 것 / 없는 것
4. 없는 것에 **핵심 흐름의 부품이 있으면 거기서 멈춘다.**
   비슷한 자산으로 대체하지 않는다. 사람에게 보고하고 결정을 기다린다.
5. **발견까지만.** 레시피는 쓰지 않는다. 만들려면 아래 "신규 컴포넌트 추가"로 넘어간다.

> 한 번만 나오는 요소는 자산이 아니라 그 화면 전용입니다. 반복 횟수가 판단 근거입니다.

> ⚠️ **테마는 색·치수만 갈아끼우는 것이 아닙니다.** 업종이 바뀌면 **자산 목록 자체가 바뀝니다.**
> 호텔 테마에는 예약 자산이 있어야 하고 커머스 테마에는 없어도 됩니다.
> `core`가 공유하는 것은 파운데이션(활자 역할·간격·모션·반응형 범위)이지 컴포넌트 목록이 아닙니다.

> ⚠️ **실제로 이 관문을 건너뛴 적이 있습니다.** 고객사 테마를 뜰 때 값(색·치수)만 재고
> **자산 대조(2~4단계)를 하지 않았습니다.** 그 결과 호텔 사이트인데 **예약 자산이 하나도 없다는 것을
> 모른 채** `form-field-md`·`select-md`로 일반 폼을 만들어 그 자리를 메꿨습니다.
> 화면은 멀쩡히 나왔고, 그래서 더 늦게 발견됐습니다.
> **"일단 되니까 넘어간다"가 가장 위험한 신호입니다.** §0-2 는 그럴 때 그리지 말고 보고하라는 조항입니다.

**분석은 표 세 개로 낸다 — 빠짐없이 · 그대로 · 바꿀 것**

우리는 대행사입니다. 역할이 둘이고, **둘 다 해야 합니다.**
하나는 고객사 사이트의 모든 요소를 빠짐없이 아는 것, 다른 하나는 **더 나은 디자인·UX 를 제안하는 것**입니다.

| 갈래 | 무엇 | 다음 행동 |
|---|---|---|
| ① **반드시 있어야 하는 것** | 핵심 업무 흐름의 부품 (호텔=예약) | manifest 에 없으면 **멈추고 보고** |
| ② **그대로 가져갈 것** | 브랜드 인상을 만드는 값 — 색·활자·라운드·간격 | 테마로 실측해 옮긴다 |
| ③ **바꾸자고 제안할 것** | 지금 사이트의 약점 — 동선·위계·반응형·접근성·밀도 | **근거와 함께 제안**한다 |

**③이 비어 있으면 우리는 복사기입니다.** 값은 ②를 지키면서 ③을 내는 데 있습니다.

**약점은 취향이 아니라 측정으로 찾는다**

| 축 | 재는 법 |
|---|---|
| 접근성 | 대비비 계산 · 터치 영역 44 · 포커스 링 · `alt` |
| 반응형 | 375 · 768 · 1024 에서 가로 넘침 · 태블릿 구간 방치 여부 |
| 동선 | 핵심 업무까지 클릭 수 · 첫 화면에서 그 입구가 보이는지 |
| 정보 위계 | 제목만 이어 읽어 페이지가 읽히는지(§7-2) |
| 밀도·리듬 | 같은 구성이 반복되는지 · 사진이 놀고 있는지 |
| 활자 | 본문 크기 · 한 줄 길이 · 굵기 대비 |

⚠️ **②와 ③을 섞지 않습니다.** "색깔은 유지하되 레이아웃은 새로" 같은 주문이 오면
**어느 축이 ②이고 어느 축이 ③인지 표로 갈라 놓고 시작합니다.**
안 그러면 실측값을 바꿔 브랜드를 훼손하거나, 약점을 그대로 베껴 제안이 없어집니다.

⚠️ **실제로 ③을 통째로 비운 적이 있습니다.** 자산 규격과 실측값은 다 맞췄는데
섹션 일곱 개가 전부 같은 틀(제목 → 리드 → 그리드)이었고, 받은 사진 26장 중 한 장만 크게 썼습니다.
고객이 받은 인상은 "느낌이 없다" 였습니다. **규격 준수는 ②의 일이고, 인상은 ③에서 나옵니다.**
...
| `hero-split-lg` | 첫 화면. 제목·리드·액션 + 지표 레일. **사진 없음** |
| `hero-media-lg` | 첫 화면. 전면 사진 위에 제목·리드·액션. 스크림 필수 |
...
> **히어로는 둘 중 하나만 씁니다.** 수치를 앞세우면 `hero-split-lg`, 장소·사진을 앞세우면 `hero-media-lg`입니다. 한 화면에 첫 화면은 하나뿐이므로 둘을 같이 놓지 않고, `hero-split-lg` 밑에 사진을 덧붙이지도 않습니다 — 그러려면 `hero-media-lg`입니다.
...
**히어로는 GNB 바로 다음입니다.** 검색 바·필터·예약 위젯 같은 **도구는 히어로 위가 아니라 아래**에 둡니다.
사람은 "여기가 어디인지" 먼저 알고 나서 조작합니다. 도구가 첫 화면이면 브랜드가 사라집니다.

⚠️ **브리프에 적힌 섹션 나열 순서를 배치 순서로 읽지 않습니다.** 실제로
`GNB / 검색 / 히어로 / …` 라고 적힌 목록을 그대로 따라 검색 폼을 히어로 위에 놓은 시안이 나왔습니다.
나열은 "이런 것들이 들어간다"는 뜻이지 "이 순서로 쌓아라"가 아닙니다.
**뼈대 순서가 먼저이고, 브리프의 나머지 항목을 그 사이에 끼웁니다.**
순서가 정말 특이해야 하는 이유가 있으면 그리기 전에 물어봅니다.
...
`variantOf`로 부모에 매달려 있어 자산 수(31)에 세지 않고, 엘루온 시트와 빌더 목록에도 나오지 않습니다.
...
2. **시간을 숫자로 쓰지 않는다.** 축약형 여섯 중에서 고른다 —
   ...
   **등장(`--motion-entrance`)만 그보다 길다.** 처음 나타나는 것은 여유가 있어야 눈에 밟힌다.
   실제 ms 는 `foundationByTheme.<테마>.motion` 에 있다 — **이 문서에 적힌 숫자가 아니라 토큰이 정답이다.**
   ⚠️ 이 문단은 오래 `420ms` 라고 적혀 있었는데 `core.json` 의 값은 `640` 이었다.
   문서의 숫자와 토큰이 어긋나면 §6 전체가 검증 불가능해진다. **숫자를 문서에 다시 적지 않는다.**
...
5-1. **스크롤 진입 등장 — 규격대로 쓴다.** 이것도 금지하지 않는다.
   ...
   | 시차 | 한 섹션 안에서 나란한 것들만 `var(--motion-stagger)` 씩. **최대 4개까지** |
