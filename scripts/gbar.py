...
/* ── 상단 내비게이션 — scripts/gbar.py 가 정본입니다 ────────────────
   세 페이지(프롬프트 빌더 · 디자인 시스템 · 사용설명서)가 같은 것을 씁니다.
...
# 로고를 누르면 가는 곳. 세 번 바뀌었으니 이유를 다 남깁니다.
#   2026-09-07  빌더 — 사람이 실제로 일을 하는 화면이라서.
#   2026-09-08  첫 화면 — 로고는 홈으로 간다는 기대가 그보다 강해서.
#   2026-09-08  다시 빌더 — 같은 날 상단바 순서를 바꿔 빌더가 맨 앞에 왔습니다.
#               맨 앞 항목이 곧 첫 화면이라, 로고와 첫 항목이 같은 곳을 가리키는 것이
#               「홈으로 간다」는 기대와도 어긋나지 않습니다. PAGES 순서를 되돌리면
#               이 값도 같이 되돌려야 합니다.
LOGO_HREF = "prompt-builder.html"

# 2026-09-08 지시로 빌더를 맨 앞에 둡니다 — 사람이 실제로 일을 하는 화면이 빌더입니다.
# 순서만 바꾸고 번호는 붙이지 않습니다.
PAGES = (
